...
def _make_plan_node(
    ...
    async def plan(state: AgentState) -> dict:
        ...
        # M9.9: Skip entirely for UPDATE mode — patterns must NOT be applied over
        # existing flows; operation_mode is set by classify_intent (M9.6).
        ...
        if (
            ...
            and state.get("operation_mode") != "update"
        ):
        ...
        # M9.9: Always emit pattern_metrics to debug so telemetry can distinguish
        # "no pattern store" from "pattern store present but no match" vs "update mode".
        _debug_update_flowise = (state.get("debug") or {}).get("flowise") or {}
        _debug_update_flowise = {
            **_debug_update_flowise,
            "pattern_metrics": {
                "pattern_used": _pattern_base_ir is not None,
                "pattern_id": _pat.get("id") if _pat_matches else None,
                "ops_in_base": (
                    len(_pattern_base_ir.get("nodes", []))
        ...
        ret: dict[str, Any] = {
            ...
            # M9.9: pattern_metrics always emitted (even when pattern_used=False)
            "debug": {"flowise": _debug_update_flowise},
            "pattern_used": _pattern_base_ir is not None,
            "pattern_id": _pat.get("id") if _pat_matches else None,
        }
...
def _repair_schema_local_sync(
    missing_node_types: list[str],
    node_store,
) -> list[str]:
    """Attempt to repair missing node schemas via local store only (synchronous).

    Checks the local NodeSchemaStore index for each missing type. Returns the
    list of node types that were found. Types absent from the local index cannot
    be repaired here — callers that need async/API repair should use
    _repair_schema_for_ops() (async) or node_store.get_or_repair() directly.

    This is the SYNCHRONOUS helper called from the v2 repair_schema graph node.
...
def _make_repair_schema_node(
    ...
    async def repair_schema(state: AgentState) -> dict:
        ...
        locally_found = _repair_schema_local_sync(missing_node_types, node_store)
...
def build_graph(
    ...
    Args:
        ...
        domains:       List of DomainTools plugins. e.g. [FloviseDomain(client)].
                       For multi-domain: [FloviseDomain(client), WorkdayDomain(workday_client)].
        checkpointer:  LangGraph checkpointer for persistence + HITL support.
                       ...
                       See DESIGN_DECISIONS.md — DD-010, DD-078.
        client:        Optional FlowiseClient passed to pattern auto-save on DONE.
        ...
        capabilities:  Optional list of DomainCapability instances (DD-046).
                       When provided, structured discovery + IR compiler path active.
